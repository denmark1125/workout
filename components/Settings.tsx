...
const Settings: React.FC<SettingsProps> = ({ profile, setProfile, onReplayOnboarding }) => {
  ...
  const handleTestConnection = async () => {
    ...
    // Fix: testConnection expects 0 arguments
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto space-y-10 pb-48 px-4 animate-in fade-in duration-500 relative">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pointer-events-none md:pl-64 pb-8 pt-12 bg-gradient-to-t from-white via-white/80 to-transparent">
        <button 
          onClick={handleSave}
          className="pointer-events-auto bg-black text-[#bef264] px-12 py-4 rounded-full font-black uppercase tracking-[0.2em] text-xs hover:bg-[#bef264] hover:text-black transition-all shadow-2xl hover:scale-105 active:scale-95 flex items-center gap-3 border-2 border-[#bef264]/20"
        >
          {showSaved ? <><CheckCircle size={16} /> 設定已同步 SYSTEM_UPDATED</> : <><Save size={16} /> 儲存變更 SAVE_CONFIG</>}
        </button>
      </div>
    </>
  );
